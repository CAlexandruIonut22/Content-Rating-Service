...
# Încărcare conținut - ACTUALIZAT pentru a suporta extragerea din URL
@app.route('/upload', methods=['GET', 'POST'])
def upload_content():
    """Încărcare conținut nou cu suport pentru URL-uri"""
    # Trebuie să fii logat
    if 'user_id' not in session:
    ...
    if request.method == 'POST':
        ...
        if 'file' in request.files and request.files['file'].filename:
            ...
            flash('Te rog să încarci un fișier sau să pui un link.')
            return redirect(url_for('upload_content'))
